...
<!-- ======= 모바일 탐색 토글 버튼 ======= -->
 ...
 <!-- ======= Hero Section ======= -->
  ...
  <!-- 폼안에 있는 모든 글자 문자열로 넘어감 <- application/x-www-form-urlencoded  -->
	<form enctype="multipart/form-data" action="<%=request.getContextPath()%>/ebook/updateEbookImgAction.jsp?ebookISBN=<%=ebookISBN%>" method="post"> <!-- 데이터로 넘어감  -->
		<input type="hidden" name="ebookISBN" value="<%=ebookISBN%>">
		<input type="file" name="ebookImg" class="form-control"> 
		<br>
		<button type="submit" class="btn btn-outline-dark btn-sm">이미지 수정</button>
